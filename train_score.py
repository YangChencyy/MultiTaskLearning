...
def calculate_loss_binary(class_weights, scores, label):  #scores are predicted outputs
    ...
    """
    #pred = torch.max(scores.data, dim=1)[1]
    #print(scores,label)
    loss1 = nn.CrossEntropyLoss(weight = class_weights)
    #loss2 =nn.MSELoss()
    return loss1(scores, label.long())
    #return torch.sqrt(loss2(scores, label.float()))
...
def train_model_score(net, trn_loader, val_loader, optim, class_weights, num_epoch=50,
    ...
    for epoch in range(num_epoch):
        ...
        for x, label in trn_loader:
            num_itr += 1
            test,aduio, label = x[0].to(device),x[1].to(device), label[1].to(device)
            ############ TODO: calculate loss, update weights ############
            optim.zero_grad()
            output = net(test,aduio)
            # print(output.shape)
...
def calculate_loss_score(class_weights, scores, label):  #scores are predicted outputs
    """
    Calculate the loss.
    Input:
        - class_weights: weight for each class.
...
def get_validation_performance_binary(net, class_weights, data_loader, device):
    ...
    with torch.no_grad():
        for x, label in data_loader:
            test,audio, label = x[0].to(device),x[1].to(device), label[1].to(device)
            ...
            output = net(test,audio)
            #print(output)
            # print(torch.max(output[0].data, dim=1))
            #print(len(output))
            pred=torch.max(output.data, dim=1)[1]
            #print(output)
            loss = calculate_loss_score(class_weights, pred, label)
    ...
    uar = recall_score(y_true, y_pred, labels=list(range(8)), average='macro')
    #uar = []
    # print(" ")
    # print(y_true, y_pred)
    w_f = f1_score(y_true, y_pred, labels=list(range(2)), average='weighted')
    confusion_mat = confusion_matrix(y_true, y_pred, labels=list(range(8)))
    #confusion_mat = []
